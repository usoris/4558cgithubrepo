...
void strafe(short value)
{
	motor[omni_Back] = value;
}
void lift()
{
	motor[lift_bot_R] = -100;
	motor[lift_bot_L] = -100;
	motor[lift_top_R] = -100;
	motor[lift_top_L] = -100;
}
...
void drive()
{
	if(vexRT(Ch3) < - drivingThreshhold || vexRT(Ch3) > drivingThreshhold)
	{
		left(vexRT(Ch3));
	}
	if(vexRT(Ch2) < - drivingThreshhold || vexRT(Ch2) > drivingThreshhold)
	{
		right(vexRT(Ch2));
	}
	/*if(vexRT(Ch2) < - drivingThreshhold || vexRT(Ch2) > drivingThreshhold)
...
void drive()
{
	if(vexRT(Ch1) < - drivingThreshhold || vexRT(Ch1) > drivingThreshhold)
	{
		chan1 = vexRT(Ch1);
		strafe(chan1);
	}
